...
export const ShateButton = () => {
  ...
  return (
    <div>
      <CustomButton
        className="bg-[#FFED00] text-[#14181F] hover:bg-[#FFFF33] h-[36px] w-[160px] rounded-[50px] flex items-center justify-center py-[6px] group max-2xl:w-[50%] xl:max-2xl:w-[100%] xl:max-2xl:px-1 xl:max-2xl:h-[30px]"
        onclick={() => setVisible(visible ? false : true)}
        label={
          <label className="flex gap-2 text-center w-[80%] text-[#14181F] font-normal leading-[20.8px] cursor-pointer max-2xl:text-[12px] items-center justify-center   xl:max-2xl:w-[100%]">
            QR Sharing
            <span className="flex items-center justify-center">
              {visible ? (
                ...
                <ShareCircle
                  className={"group-hover:hidden  xl:max-2xl:w-[80%]"}
                />
              )}
              <span
                className={`hidden ${
                  visible ? "hidden" : "group-hover:block xl:max-2xl:px-[5px]"
                }`}
              >
                {"\u25B2"}
              </span>

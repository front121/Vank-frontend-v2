        ...
        <div className="h-full flex flex-col  pb-[0px] justify-between group-hover:w-full ">
          ...
          <ul className="relative flex flex-col xl:h-[170px] xl:py-3 xl:justify-between  group-hover:xl:h-[220px] md:h-[170px] md:justify-between group-hover:gap-[20px]   w-[186px]   justify-end ">
            {listLinksFooter.map((item) => (
             <div className={` pl-5 ${item.id!=3? 'group-hover:ml-2 cursor-pointer group-hover:bg-[#3E4347] group-hover:p-[10px] group-hover:pl-[35px] group-hover:w-[169px] group-hover:rounded-[33px]  ':'group-hover:pl-[0px]'} `}
             onClick={item.id===1?()=>(setVisibleSidebar(()=>{
              visibleSidevar?setVisibleSidebar(false):setVisibleSidebar(true)
             })):''} >
                <button className={ `text-[#EFF0F1] ${item.id!=3?'flex justify-around gap-2':''}`} 
                 onClick={item.id===1?()=>(setVisibleSidebar(()=>{
                  visibleSidevar?setVisibleSidebar(false):setVisibleSidebar(true)
                 })):''}>
                  <p className={`${item.id!=3?'':'group-hover:hidden '}`}>{item.icon}</p>
                  <p className={`${item.id!=3?'':'text-[12px] font-[700] leading-[15.6px]  text-center w-[217px] relative left-[-14px]'} hidden group-hover:block`}>{item.text}</p>
                </button>
             </div>
            ))}
            ...
            <ul
              className={`${
                visibleSidevar ? "block opacity-0 group-hover:opacity-100": "hidden "
